...
def points_inside_convex_hull(point_cloud, mask, remove_outliers=True, outlier_factor=1.0):
    ...
    if remove_outliers:
        ...
        filtered_outlier_mask = np.any(outlier_mask, axis=1)
        filtered_outlier_mask_tensor = torch.tensor(filtered_outlier_mask, device='cuda', dtype=torch.bool)
        filtered_mask[mask] = ~filtered_outlier_mask_tensor
...
def removal_setup(opt, model_path, iteration, views, gaussians, pipeline, background, classifier, selected_obj_ids, cameras_extent, removal_thresh):
    ...
    with torch.no_grad():
        ...
        print("prob_obj3d",prob_obj3d.shape)
        print("prob_obj3d[0]: ",prob_obj3d[selected_obj_ids])
        print("removal_thresh",removal_thresh)
        remocal_thresh = 0.03
    ...
    print("gaussians._xyz",gaussians._xyz.shape)
...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, classifier):
    print("render gaussians._xyz",gaussians._xyz.shape)
...
def removal(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, opt : OptimizationParams, select_obj_id : int, removal_thresh : float):
    ...
    # 3. render new result
    print("scene.loaded_iter",scene.loaded_iter)
    scene.loaded_iter = iteration
    print("1 : gaussian._xyz",gaussians._xyz.shape)
    scene = Scene(dataset, gaussians, load_iteration=-2, shuffle=False)
    print("2 : gaussian._xyz",gaussians._xyz.shape)
    with torch.no_grad():
        if not skip_train:
...
if __name__ == "__main__":
    ...
    args.removal_thresh = config.get("removal_thresh", 0.03)
    args.select_obj_id = config.get("select_obj_id", [1])
